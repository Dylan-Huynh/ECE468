...
class CodeGenerator(AbstractASTVisitor):
  ...
  def postprocessFloatLitNode(self, node: FloatLitNode) -> CodeObject:
    ...
    if type(node.getType()) is Scope.InnerType:
      co.type = node.getType()
    else:
  ...
  def postprocessBinaryOpNode(self, node: BinaryOpNode, left: CodeObject, right: CodeObject) -> CodeObject:
    ...
    #step 3
    opType = node.getOp()
    if type(left.getType()) is Scope.InnerType:
      leftType = left.getType()
    else:
      leftType = left.getType().type
      
    if type(right.getType()) is Scope.InnerType:
      rightType = right.getType()
    else:
      rightType = right.getType().type
    
    if (leftType is Scope.InnerType.FLOAT or rightType is Scope.InnerType.FLOAT):
      if (leftType is Scope.InnerType.INT or "t" in left.temp):
        oldtemp = left.temp
        left.temp = self.generateTemp(Scope.InnerType.FLOAT)
        il.append(IMovF(oldtemp, left.temp))
        co.type = rightType
      elif (rightType is Scope.InnerType.INT):
        oldtemp = right.temp
        right.temp = self.generateTemp(Scope.InnerType.FLOAT)
        il.append(IMovF(oldtemp, right.temp))
        co.type = leftType
      else:
        co.type = leftType
  ...
  def postprocessUnaryOpNode(self, node: UnaryOpNode, expr: CodeObject) -> CodeObject:
    ...
    if type(expr.getType()) is Scope.InnerType:
  ...
  def postprocessAssignNode(self, node: AssignNode, left: CodeObject, right: CodeObject) -> CodeObject:
    ...
    if type(left) is InstructionList:
      if nodeType is Scope.InnerType.FLOAT:
        if rightType is Scope.InnerType.INT:
          oldtemp = right.temp
          right.temp = self.generateTemp(Scope.InnerType.FLOAT)
          il.append(IMovF(oldtemp, right.temp))
        ...
        if rightType is Scope.InnerType.FLOAT:
          oldtemp = right.temp
          right.temp = self.generateTemp(Scope.InnerType.INT)
          il.append(FMovI(oldtemp, right.temp))
        if("f" in right.temp):
          raise Exception("it's in 281")
  ...
  def postprocessReadNode(self, node: ReadNode, var: CodeObject) -> CodeObject:
    ...
    if type(node.getType()) is Scope.InnerType:
      ...
      typ = node.getType().type

    if typ is Scope.InnerType.INT:
      ...
      if var.getSTE().isLocal():
        ...
        if("f" in geti.getDest()):
            raise Exception("it's in 367")
      else:
        store.extend(self.generateAddrFromVariable(var))
        store.append(Sw(geti.getDest(), store.getLast().getDest(), "0"))
        if("f" in geti.getDest()):
          raise Exception("it's in 372")
      il.extend(store)
    elif typ is Scope.InnerType.FLOAT:
  ...
  def postprocessReturnNode(self, node: ReturnNode, retExpr: CodeObject) -> CodeObject:
    ...
    if (retType is Scope.InnerType.FLOAT):
      ...
      if("f" in retExpr.temp):
        #raise Exception("Rettype: %d, the last thing that happened was %d", retType, str(retExpr.code))
        co.code.append(Fsw(retExpr.temp, "fp", "8"))
        co.type = Scope.InnerType.FLOAT
        #raise Exception("It's in 706")
      else:
  ...
  def postprocessCallNode(self, node: CallNode, args: List[CodeObject]) -> CodeObject:
    ...
    for i in args:
      if (i.lval is True):
        i = self.rvalify(i)
      if type(i.getType()) is Scope.InnerType:
        iType = i.getType()
      else:
        iType = i.getType().type
      co.code.extend(i.code)
      if (iType is Scope.InnerType.FLOAT):
    ...
    if (retType.type is Scope.InnerType.FLOAT):
      co.temp = self.generateTemp(Scope.InnerType.FLOAT)
      co.code.append(Addi("sp", "-4", "sp"))
    else:
      co.temp = self.generateTemp(Scope.InnerType.INT)
 ...
 # Step 3: New code object has a "wrapped" type. If type of expr is T, type of temporary is *T. Can get this from node
  ...
  def postprocessCastNode(self, node: CastNode, expr: CodeObject) -> CodeObject:
    co = CodeObject()
    #co.code.append(Blank("For cast"))
    il = InstructionList()
    if (expr.lval is True):
      expr = self.rvalify(expr)
    co.code.extend(expr.code)

    if node.getType().type is Scope.InnerType.FLOAT:
      oldtemp = expr.temp
      expr.temp = self.generateTemp(Scope.InnerType.FLOAT)
      il.append(IMovF(oldtemp, expr.temp))
    else:
      oldtemp = expr.temp
      expr.temp = self.generateTemp(Scope.InnerType.INT)
      il.append(FMovI(oldtemp, expr.temp))
    co.code.extend(il)
    co.temp = expr.temp
    co.type = node.getType()
  ...
  def rvalify(self, lco : CodeObject) -> CodeObject:
    ...
    if lco.isVar():
      if lco.getSTE().isLocal():
        ...
        if type(lco.getType()) is Scope.InnerType:
          lcoType = lco.getType()
        else:
